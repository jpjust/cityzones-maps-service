...
Map.addLayer(fc_risks.filter(ee.Filter.eq('class', 0)).draw({color: 'red',  pointRadius: 1}), {opacity: 0.5}, 'Risk Level 0');
Map.addLayer(fc_risks.filter(ee.Filter.eq('class', 1)).draw({color: 'yellow', pointRadius: 1}), {opacity: 0.5}, 'Risk Level 1');
Map.addLayer(fc_risks.filter(ee.Filter.eq('class', 2)).draw({color: 'green',    pointRadius: 1}), {opacity: 0.5}, 'Risk Level 2');
